	...
	*/
	...
	function renderNode(node){
		...
		if(node.isLeaf() && !TreeUtil.getProperty(node.getOptions(), "forceParent", false)){
			...
			if(icon != ""){
				ret += '<span class="tj_icon">' + `<span class="badge badge-success">Skill</span>` + '</span>';
			...
			var icon = TreeUtil.getProperty(node.getOptions(), "icon", "");
			if(icon != ""){
				ret += '<span class="tj_icon">' +`<span class="badge badge-primary ml-3">Main Skills</span>`+  '</span>';
			...
			span_desc.innerHTML = ret + node.toString() + '</span>';
			span_desc.classList.add("tj_parent");
