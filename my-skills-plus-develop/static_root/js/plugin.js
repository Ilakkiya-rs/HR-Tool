var isLoginUser = JSON.parse(
  document.getElementById("is_login_user").textContent
);

const ENDPOINT_URL =
  "https://5fr8r16ski.execute-api.us-east-1.amazonaws.com/stage";
...
class IysSearchPlugin {
  ...
  searchAPI() {
    ...
    if (this.searchValue.length > 1) {
      fetch(`${ENDPOINT_URL}/dev-api/ISOT/?q=${this.searchValue}&limit=10`)
        .then((response) => {
          if (response.status === 429) {
            // Redirect to /limit-exceeded/ page
            window.location.href = "/limit-exceeded/";
...
class IysFunctionalAreasPlugin extends IysSearchPlugin {
  ...
  init() {
    ...
    this.softLanguageProficiencySkillAPI();
    this.ExperienceProfileAPI();
  ...
  SkillChildrenAPI(skillFileId) {
    fetch(
      window.location.origin + "/api-child/?path_addr=" + skillFileId,
      this.rapidAPIheaders
    )
  ...
  saveTheSkillComment(commentValue, ratingValue, skillDetail) {
    ...
    if (isLoginUser) {
      ...
      fetchData(`${ENDPOINT_URL}/dev-api/ISOT/details/?ids=${skillDetail._id}`)
        .then((response) => {
          addTolocalStorage({
            comment: commentValue,
            rating: ratingValue,
  ...
  functionalAreaAPI() {
    ...
    if (isLoginUser) {
      ...
      url = `${ENDPOINT_URL}/dev-api/ISOT/popular-categories/`;
    }
  ...
  softLanguageProficiencySkillAPI() {
    ...
    if (isLoginUser) {
      ...
      url = `${ENDPOINT_URL}/dev-api/ISOT/children/?path_addr=${skillId}`;
    }
  ...
  treeSkillAPI(cardBodyDiv, skillId) {
    ...
    if (isLoginUser) {
      ...
      url = `${ENDPOINT_URL}/dev-api/ISOT/tree/?path_addr=${skillId}`;
    }
