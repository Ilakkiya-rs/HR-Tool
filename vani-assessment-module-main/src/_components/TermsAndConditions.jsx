...
const TermsAndConditions =({token,email}) => {
  ...
  async function handleAgree() {
    ...
    try {
      const res = await fetch(`https://api.myskillsplus.com/initial-voice/${token}/`, {
      // const res = await fetch(`http://127.0.0.1:8000/initial-voice/${token}/`, {
        method: "POST",
        body: formData,
      });
  ...
  if (mode === "voice_page"){
    ...
    <div className="w-screen min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-blue-100 px-24">
      ...
      <p className="text-lg bg-white p-8 mb-8" id="required">
        I agree to use my own voice for this interview and acknowledge that my voice will be recorded and stored for future verification.<br/>
        I understand that AI-based analysis may be used to verify the authenticity of my responses.<br/>
        Any impersonation or mismatch may result in suspension or permanent removal from the platform.<br/>
        I acknowledge that the interview must be completed in one sitting and can only be resumed once within 24 hours if interrupted.<br/>
        I consent to the use of my voice metadata for training, analytics, and system improvement purposes.<br/>
        I understand I may request withdrawal of my data.
      </p>

      {!recording && !verified && <button className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-700 hover:from-blue-700 hover:via-purple-700 hover:to-indigo-800 text-white font-semibold px-6 py-3 rounded-xl text-lg hover:cursor-pointer" onClick={startRecording}>Start Recording</button>}
      {recording && !verified && <button className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-700 hover:from-blue-700 hover:via-purple-700 hover:to-indigo-800 text-white font-semibold px-6 py-3 rounded-xl text-lg hover:cursor-pointer" onClick={stopRecording}>Stop Recording</button>}
  ...
  if(mode === "voice_selection"){
    return <VoiceSelection data={voiceData}/>
  }
}
