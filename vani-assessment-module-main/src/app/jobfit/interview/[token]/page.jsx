...
export default function Verify() {
  ...
  useEffect(() => {
    ...
    if (window.opener) {
      window.opener.postMessage("READY", "https://job.myskillsplus.com");
      // window.opener.postMessage("READY", "http://localhost:3000");
    }

    const handler = (event) => {
      if (event.origin !== "https://job.myskillsplus.com") return;
  ...
  const validateForm = () => {
    ...
    if (!languageCode.trim()) newErrors.name = "Please choose language first";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://api.myskillsplus.com/jobfit/interview_link/${token}/`, {
      // const response = await fetch(`http://127.0.0.1:8000/jobfit/interview_link/${token}/`, {
        method: "POST",
  ...
  if (mode === "start") {
    return <EmailVerification token={token} languageCode={languageCode}/>
  }

  if (mode === "resume") {
    return <ResumeInterview token={token} email={formData.email} name={formData.name} languageCode={languageCode}/>
  }

  if (mode === "continue") {
    return <TermsAndConditions token={token} email={formData.email} languageCode={languageCode}/>
  }
