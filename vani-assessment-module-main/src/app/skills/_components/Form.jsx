...
const Form = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!personalInfo.email?.trim()) {
      setMessage("Email is required to generate an assessment.");
      setType("error");
      return;
    }
    ...
    if (!formattedSkills.length) {
      setMessage("Add at least one skill with a name and proficiency rating.");
      setType("error");
      return;
    }
    setMessage("")
    ...
    try {
      const response = await fetch(`${API_BASE}/skills/generate-assessment/`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        setMessage(assessmentData.error || assessmentData.detail || `Server error (${response.status})`);
        setType('error')
      } else {
        window.location.href = assessmentData.link;
      ...
      setIsProcessing(false);
      setMessage("There was an error in generating your assessment. Please try again...");
      setType('error')
    }
  };
