...
import { useAuth } from "@/components/common/checkAuth";
import { API } from "../../app/auth/endpoints";
import { MaterialSymbol } from "react-material-symbols";

const DropdownUser = () => {
  const { userDetails, tokens } = useAuth();
  ...
  const goToProfile = async () => {
    window.location.href = `/profile/${shareId}`;
  };
  ...
  const handleLogout = () => {
    ...
    setTimeout(() => {
      ...
      // localStorage.removeItem("logedinUserDetail");
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <div className="elative flex h-8.5 w-8.5 items-center justify-center rounded-full border-[0.5px] border-stroke bg-gray hover:text-primary dark:border-strokedark dark:bg-meta-4 dark:text-white">
          <svg
      ...
      >
        <ul className="flex flex-col gap-5 border-b border-stroke dark:border-strokedark">
          <li>
            <button
              onClick={() => {
                goToProfile();
