...
import { VANI_SKILLS_PAGE_URL } from "@/config/externalApps";
...
const HomePageContent = () => {
    ...
    if (sessionId) {
        ...
        .then(data => {
          if (data.is_payment_complete)  {
            ...
            localStorage.setItem('logedinUserDetail', JSON.stringify(userDetails));
      ...
      const storedUserDetails = JSON.parse(localStorage.getItem('logedinUserDetail'));
  ...
  const handleOpenVaniAssessment = () => {
    const skillsRaw =
    ...
    const userData = JSON.parse(localStorage.getItem("logedinUserDetail") || "null");
    ...
    const assessmentWindow = window.open(VANI_SKILLS_PAGE_URL, "_blank");
    if (!assessmentWindow) {
      alert("Pop-up blocked. Allow pop-ups for this site to open Vani assessment.");
      return;
    }
  ...
  return (
    <>
      <div className="flex gap-3 no-print">
        ...
        <button
          type="button"
          onClick={handleOpenVaniAssessment}
          className={`rounded bg-gradient-to-r from-indigo-600 to-purple-600 px-4 py-2 font-bold text-white hover:from-indigo-700 hover:to-purple-700 ${
            !isPaid ? "opacity-50 cursor-not-allowed" : ""
          }`}
        >
          Vani assessment
      ...
      <div style={{ display: "none" }}>
        {iframeContent && quickTabContent && (
          <PrintPage ref={printRef} tabularViewContent={iframeContent} quickTabContent={quickTabContent} />
        )}
      </div>
    </>
  );
