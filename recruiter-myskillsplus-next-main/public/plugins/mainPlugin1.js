...
const config = JSON.parse(localStorage.getItem('iys'));
console.log(config);
...
class IysFunctionalAreasPlugin extends IysSearchPlugin {
    ...
    setupDiv() {
        ...
        if(config.page=="Home"){
            ...
            cardDiv.className = "bg-transparent";
        }

        // Create nav element
        var navElement = document.createElement("nav");
        if(config.page=="Home" || config.page=="Feedback"){
        ...
        if(config.tap=="search"){
            ulElement.appendChild(homeLi);
        }
        else if(config.tap=="profile"){
            ulElement.appendChild(profileLi);
        }
        else if(config.tap=="all"){
            ulElement.appendChild(homeLi);
            ulElement.appendChild(profileLi);
        }
        ...
        if(config.page=="Home" || config.page=="Feedback"){
            ...
            containerFluidDiv.className = "bg-transparent pb-md-3";
        }
        ...
        if(config.page=="Home" || config.page=="Export"){
            btnGroupDiv.className = "btn-group border";
        }
        else{
            btnGroupDiv.className = "btn-group border bg-white";
        }
        ...
        if(config.profile_view=="quick"){
            btnGroupDiv.appendChild(quickTabButton);
        }
        else if (config.profile_view=="tabular"){
            btnGroupDiv.appendChild(tabularTabButton);
        }
        else if(config.profile_view=="all"){
            btnGroupDiv.appendChild(quickTabButton);
            btnGroupDiv.appendChild(tabularTabButton);
        }
        ...
        if(config.page=="Home" || config.page=="Export" || config.page=="Profile"){
            ...
            pTabularView.textContent = "Presents your skills in a logical and organized way, like that in our report cards in school.";
        }
        else{
            var h5TabularView = document.createElement("p");
            h5TabularView.className = "h5";
        ...
        rateSkillCommentBoxTextarea.height = "14"
            ...
            arbitraryValuesForSlider.forEach((sliderObj) => {
                ...
                if (options.length === 2) {
                    ...
                    if (!options.includes("Zero Level Knowledge")) {
                        options.unshift("Zero Level Knowledge");
                    }
            ...
            arbitraryValuesForSlider.forEach((sliderObj) => {
                if (sliderObj.rating_scale_label.length === 2) {
                    ...
                    if (!isChecked) {
                        ...
                        toastr.error("Please select a Experience Recency.");
                    }
                } else {
                    ...
                    const indexValue = sliderObj?.rating_scale_label.indexOf(handlerValue);
                    if (indexValue !== 0) {
                        inputData.push({
                        ...
                        hasInvalidRating = true;
                        toastr.error("Please select a valid rating.");
                    }
                }
            });
            if (hasInvalidRating) {
    ...
    appendQuickViewContent() {
        ...
        for (const tagsString in groupedSkills) {
            ...
            skillsGroup.forEach((skill) => {
                ...
                const imagePathBase = '/images/percentage/';
        ...
        for (const tagsString in groupedSkills) {
            ...
            skillsGroup.forEach((skill, index) => {
                ...
                skillName.innerHTML =
                    ...
                    } <span class="badge rounded-pill badge-primary">${getExpertiseLevel(skill.rating, skill.isot_file.ratings)
...
function getExpertiseLevel(ratingValue, ratingLabel) {
    ...
    if (expertiseLevelRating) {
        ...
        if(config.page=="Feedback"){
            return expertiseLevelRating.rating_scale_label[ratingValue.rating - 1];
        }
        else{
