...
const BASE_URL = "https://api.myskillsplus.com";

const ENDPOINT_URL = "https://lambdaapi.iysskillstech.com/latest/dev-api/";
const loggedInUserApiEndpoint = BASE_URL + `/get-skills/`;
const loggedInUserAddSkill = BASE_URL + `/add-skills/`;
const deleteSkillApiEndpoint = BASE_URL + `/delete-skill/`;
const getaccessYokenEndpoint = BASE_URL + "/api/token/refresh/";
...
class IysSearchPlugin {
  ...
  searchAPI() {
    ...
    if (isLoginUser && this.searchValue.length > 0) {
      fetch(BASE_URL + `/api-search/?q=${this.searchValue.trim()}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${getAccessToken?.access}`,
...
class IysFunctionalAreasPlugin extends IysSearchPlugin {
  ...
  setupDiv() {
    ...
    containerDiv.className = "w-full mb-3";
    ...
    ulElement.className = "nav nav-tabs border-bottom";
    ulElement.id = "myTab0";
    ...
    tabContentDiv.className = "tab-content px-3";
    ...
    // h3Element.textContent = "Sample Profile";
  ...
  async init() {
    ...
    if (isLoginUser) {
      //  For rating saved
      console.warn(getListFromlocalStorage(), "sacing things");
      const transformSkillList = transformDataFromLocalStorage(
        getListFromlocalStorage(),
      );
      console.log("init called", isLoginUser, transformSkillList);
  ...
  SkillChildrenAPI(skillFileId) {
    let url;
    if (isLoginUser) {
      url = BASE_URL + `/api-child/?path_addr=${skillFileId}`;
    } else {
  ...
  changeRateModelElement(skillDetail, parentSkillDetailId) {
    ...
    console.log("objExist edit", objExist);
  ...
  childrenSkillAPI(skillId, identifier, parentIdOfHirarchy = "") {
    ...
    if (skillIdElement) {
      ...
      let url = "";
      if (isLoginUser) {
        url = BASE_URL + `/api-child/?path_addr=${skillId}`;
      } else {
        url = `${ENDPOINT_URL}children/?path_addr=${skillId}`;
      }
  ...
  treeSkillAPI(cardBodyDiv, skillId) {
    let url = "";
    if (isLoginUser) {
      url = BASE_URL + `/api-tree/?path_addr=${skillId}`;
      fetch(url, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
