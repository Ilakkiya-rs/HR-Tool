...
export default function UserMessagePage() {
  ...
  const fetchMessages = async (conversationUserId) => {
    const res = await fetch(`https://api.myskillsplus.com/messages/${conversationUserId}/${loggedInUserId}/`);
    const data = await res.json();
    setMessages(data);
  };
    ...
    <>
      {conversations.length > 0 ? (
        <div className="flex h-[75vh] gap-5">
          {/* Sidebar */}
          <div className="w-1/3 bg-white max-w-sm border rounded-2xl bg-gray-50 overflow-y-auto">
            ...
            <div>
              {conversations.map((conv) => (
                <div
                  ...
                  className={`px-4 py-3 cursor-pointer ${activeChat?.id === conv.id ? "bg-blue-50" : "hover:bg-gray-100"}`}
                >
                  <div className="font-semibold">{conv.user.id}</div>
                  <div className="text-sm text-gray-500">{conv.last_message?.text}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 bg-white flex flex-col rounded-2xl shadow-sm">
            ...
            <div className="px-4 py-3 flex items-center gap-2 relative">
              <div className="relative">
                <Smile className="text-gray-500 cursor-pointer" onClick={() => setShowEmojiPicker((prev) => !prev)} />
                {showEmojiPicker && (
              ...
              />
              <Mic className="text-gray-500 cursor-pointer" onClick={() => alert("Voice recording not implemented")} />
              <button onClick={handleSend} className="bg-[#25d366] p-2 rounded-full text-white">
                <Send size={18} />
              </button>
            </div>
          </div>
        ...
        <div className="flex flex-col items-center justify-center p-12">
          <img src="/images/logo/no-message.png" alt="No Data" className="w-48 mb-2" />
