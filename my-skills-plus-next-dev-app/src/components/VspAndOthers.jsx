...
export default function VspAndOthers() {
  ...
  const handleSave = async () => {
    if (!vspCost || !pbpFee || !pcpFee) {
      alert("Please enter values for VSP, BVF (Background View Fee), and PCF (Person Contact Fee) before saving.");
      return;
    }
    const payload = {
      userId,
  ...
  return loading ? (
    ...
    <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-4 text-sm text-[#24303F]">
            {/* VSP Section */}
            <div className="rounded-xl shadow p-4 bg-white">
                <h2 className="font-semibold mb-2">VSP (Value of Skills Profile)</h2>
                <label className="block text-sm mb-1 font-medium">Per-hour rate for work</label>
                <div className="flex border border-[#D1D5DB] rounded-lg overflow-hidden">
                    <input
                        ...
                        onChange={(e) => {
                          ...
                          if (val === "" || parseFloat(val) >= 0) setVspCost(val);
                        }}
                    />
                    <div className="p-1">
                        <select
                ...
                <div className="mt-4 p-3 bg-[#F6F6F6] rounded-lg">
                    ...
                    <input
                        ...
                        onChange={(e) => {
                          const val = e.target.value;
                          if (val === "" || parseFloat(val) >= 0) setCtc(val);
                        }}
                    />
            ...
            {/* PBP View Fee */}
            <div className="rounded-xl shadow p-4 bg-white">
                <h2 className="font-semibold mb-2">BVF( Background View Fee)</h2>
                <label className="block text-sm mb-1"><span className="font-medium">Fee to view profile background</span> (Currently taken as 10% of VSP)</label>
                <div className="flex border border-[#D1D5DB] rounded-lg overflow-hidden">
                    <input
                        type="number"
                        min={0}
                        ...
                        onChange={(e) => {
                          const val = e.target.value;
                          if (val === "" || parseFloat(val) >= 0) setPbpFee(val);
                        }}
                    />
                    <div className="p-1">
                        <select
            ...
            {/* PCP View Fee */}
            <div className="rounded-xl shadow p-4 bg-white">
                <h2 className="font-semibold mb-2">PCF (Person Contact Fee)</h2>
                <label className="block text-sm mb-1"><span className="font-medium">Fee to access contact details</span> (Currently taken as 10% of VSP)</label>
                <div className="flex border border-[#D1D5DB] rounded-lg overflow-hidden">
                    <input
                        type="number"
                        min={0}
                        ...
                        onChange={(e) => {
                          const val = e.target.value;
                          if (val === "" || parseFloat(val) >= 0) setPcpFee(val);
                        }}
                    />
                    <div className="p-1">
                        <select
